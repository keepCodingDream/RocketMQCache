...
public class DemoTester {
    ...
    private static final String PREFIX = "bbfbfewfbyw128bchhwefbbbfbfew121212fbyw128bchhwefbxx:";
            ...
            while ((count = counter.getAndIncrement()) < maxMsgNum && System.currentTimeMillis() <= maxTimeStamp) {
                try {
                    ...
                    synchronized (queueCounter.get(queueName)) {
                        queueStore.put(queueName, (PREFIX + String.valueOf(queueCounter.get(queueName).getAndIncrement())).getBytes());
                    }
                } catch (Throwable t) {
                    t.printStackTrace();
                    System.exit(-1);
        ...
        public void run() {
            ...
            while (counter.getAndIncrement() < maxMsgNum && System.currentTimeMillis() <= maxTimeStamp) {
                try {
                    ...
                    for (byte[] msg : msgs) {
                        if (!new String(msg).equals(PREFIX + String.valueOf(index++))) {
                            System.out.println("Check error queueName:" + queueName + " need index:" + (index - 1) + " act:" + new String(msg));
                            System.out.println(Math.abs(queueName.hashCode()) % Config.TOTAL_CHUNK);
                            System.exit(-1);
                        }
                    }
                } catch (Throwable t) {
                ...
                try {
                    for (String queueName : pullOffsets.keySet()) {
                        ...
                        if (msgs != null && msgs.size() > 0) {
                            ...
                            for (byte[] msg : msgs) {
                                if (!new String(msg).equals(PREFIX + String.valueOf(index++))) {
                                    System.out.println("Check error2 queueName:" + queueName + " index:" + (index - 1) + "my:" + new String(msg));
                        ...
                        if (msgs == null || msgs.size() < 10) {
                            if (pullOffsets.get(queueName).get() != offsets.get(queueName).get()) {
                                System.out.println("Queue Number Error queueName:" + queueName + " index:" + index + "size:" + msgs.size());
                                System.exit(-1);
                            }
